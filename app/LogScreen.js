...
export default function LogScreen({ onLogged }) {
  ...
  function accept(data) {
    const expenses = Array.isArray(data?.expenses) && data.expenses.length
      ? data.expenses
      : (data?.expense ? [data.expense] : []);
    if (!expenses.length) throw new Error('The server did not return a saved expense.');
    setResult({ ...data, expense: data.expense || expenses[0], expenses });
    onLogged();
  }
  ...
  const expenses = Array.isArray(result?.expenses) && result.expenses.length ? result.expenses : (result?.expense ? [result.expense] : []);
  ...
  return <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.page}>
    ...
    {expenses.length ? <View style={styles.success}><Text style={styles.successEyebrow}>{expenses.length > 1 ? `${expenses.length} EXPENSES SAVED` : 'EXPENSE SAVED'}</Text>{expenses.map((item, i) => <View key={String(item.id ?? i)} style={i > 0 ? styles.successItem : null}><Text style={styles.successAmount}>{money(item.amountCents ?? item.amount_cents ?? item.cents)}</Text><Text style={styles.successCategory}>{item.category}{item.merchant ? ` · ${item.merchant}` : ''}</Text><Text style={styles.hint}>“{item.originalText ?? item.original_text ?? ''}”</Text>{item.needsReview || item.needs_review ? <Text style={styles.error}>Please verify the detected amount and category.</Text> : null}</View>)}</View> : null}
