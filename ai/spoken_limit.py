"""Person C — spoken weekly-limit understanding via Nemotron JSON.

``understand_spoken_limit(raw_text)`` returns
``{category, amount_cents, period: weekly, confidence}``.
Uses ``ask_nemotron_json`` (already soft-loads ``NVIDIA_API_KEY``).
On failure returns a fully-null, zero-confidence result — never crashes.
"""
